...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_CarDet_Gru_Crit_Sal_Min_Meses.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 22/09/2022
# MAGIC * Descripcion: Evaluacion cantidad minima de meses de permanencia.
...
# MAGIC %md
# MAGIC ### Clientes con Minimo de Meses
# MAGIC --------------------------------------
# MAGIC - Se extrae operaciones para evaluar condiciones de salida

# COMMAND ----------

#Extrae las operaciones de la tabla tbl_c4_tmp_gru_ope_ent_meses para el periodo actual con minimo de meses
paso_query20 = f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_EXT_tbl_gru_ope_ent_meses AS
SELECT 
  b.fec_proc                        AS fecha_cierre,
  b.sistema                         AS cod_sistema,
  b.num_interno_ident               AS operacion,
  b.rut_cli                         AS rut_cliente,
  b.num_meses                       AS num_meses
FROM
  tbl_c4_tmp_gru_ope_ent_meses b
WHERE 
  b.fec_proc = {FechaX} 
"""
...
# MAGIC %md
# MAGIC ### Operaciones con minimo de meses
...
SELECT
     ...
     A.dv_rut_cliente ,
     A.tipo_operacion,
    'num_meses' AS nombre_campo,
     IFNULL(B.num_meses,0) AS valor_campo,
     ">= "  AS condicion_regla,     
     {p_CANTMINMES}  AS valor_regla,           
     CASE WHEN trim (B.num_meses) >= {p_CANTMINMES} THEN 1 ELSE 0 END AS flag_resultado_regla
FROM 
    tmp_EXT_tbl_cartdet_crit_ent_ope_prin A 
LEFT JOIN 
    tmp_EXT_tbl_gru_ope_ent_meses  B
ON  
...
paso_query50 = f"""DELETE FROM {base_silverX}.tbl_cartdet_crit_sal_ope_campo where fecha_cierre = {FechaX} AND criterio_salida = {p_GruCritDetSal} AND tipo_proceso = '{tipo_procesoX}' """
...
# MAGIC %sql
# MAGIC --select count(1) from tmp_tbl_cartdet_crit_sal_ope_campo where criterio_salida = 66  
# MAGIC select count(1) from riesgobdu_silver_db.tbl_cartdet_crit_sal_ope_campo where criterio_salida = 66 and flag_resultado_regla = 1