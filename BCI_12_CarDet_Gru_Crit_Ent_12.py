...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_12_CarDet_Gru_Crit_Ent_12.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 01/09/2022
# MAGIC * Descripcion: Clientes SSFF. Criterio 12.
...
# MAGIC ********************************************************************************************************
# MAGIC ##### Pendientes:
# MAGIC * Se ingesta temporalmente el archivo SSFF
...
paso_query15 = f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_EXT_tbl_cartdet_SSFF AS
SELECT
    rut                  AS rut_ssff
    ,dv                  AS dv_ssff
    ,nope                AS nope_ssff
    ,mot                 AS mot_ssff
    ,finidet             AS finidet_ssff
    ,finimor             AS finimor_ssff
    ,dmor                AS dmor_ssff
FROM
    tbl_cartdet_SSFF
"""
...
paso_query20 =  f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_RES_D00_OPE_SSFF_ACT AS 
SELECT  
    ...
    ,IFNULL(A.rut_ssff,0)                  AS rut_ssff 
	,IFNULL(A.dv_ssff,' ')                 AS dv_ssff    
	,IFNULL(B.tipo_operacion,' ')          AS tipo_operacion
FROM 
  tmp_EXT_tbl_cartdet_SSFF  A
LEFT JOIN
  tmp_EXT_tbl_segmentacion_d00_segmentado B
ON
  A.rut_ssff = B.rut_cliente
"""
...
SELECT
   ...
   'rut_ssff'                           AS nombre_campo,
   cast(A.rut_ssff as string)           AS valor_campo,
   " EXIST "                            AS condicion_regla,     
   a.rut_ssff                           AS valor_regla,           
   "1"                                  AS flag_resultado_regla
FROM 
    tmp_RES_D00_OPE_SSFF_ACT A
"""
