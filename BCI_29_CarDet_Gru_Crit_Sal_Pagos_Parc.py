...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_CarDet_Gru_Crit_Sal_Pagos_Parc.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 23/09/2022
# MAGIC * Descripcion: Criterio de Salida de Cartera Deteriorada para clientes que tengan operaciones con pagos parciales.
...
# MAGIC %md
# MAGIC ### Clientes con operaciones con pagos parciales
# MAGIC --------------------------------------
...
#Extrae nro_ctas_amort_pend de la tabla tbl_c4_dat_ope_ini_cd para el ultimo periodo informado
paso_query30 = f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_EXT_tbl_c4_dat_ope_ini_cd AS
SELECT
     a.fec_proc
     ,a.sistema 
     ,a.num_interno_ident
     ...
     ,a.rut_cli 
     ,a.fec_ini_cd
     ,a.nro_ctas_amort_pend 
     ,a.val_skif
     ,a.ori_det
FROM 
	tbl_c4_dat_ope_ini_cd a 
	,(SELECT MAX(fec_proc) AS fec_proc,sistema,num_interno_ident,rut_cli FROM tbl_c4_dat_ope_ini_cd GROUP BY sistema,num_interno_ident,rut_cli) b 
WHERE 
	a.fec_proc = b.fec_proc 
	AND a.num_interno_ident = b.num_interno_ident 
...
SELECT 
  ...
  (substring(b.rut_cliente,2,8))        AS rut_cliente,
  (substring(b.dv_cliente,10,1))        AS dv_cliente,
  b.saldo_total_ifrs                    AS saldo_total_ifrs,
  b.cuotas_amort_por_vencer             AS cuotas_amort_por_vencer
FROM
  riesgobdu_silver_db.tbl_d00_segmentado_gnz b
WHERE 
...
SELECT
     ...
     A.dv_rut_cliente ,
     A.tipo_operacion,
    'pagos_parciales' AS nombre_campo,
     (IFNULL(D.nro_ctas_amort_pend,0) - IFNULL(C.cuotas_amort_por_vencer,0)) AS valor_campo,
     " >= "  AS condicion_regla,     
     {p_cant_min_pag}  AS valor_regla,           /* Este valor debe ser parametrico */
     CASE WHEN (IFNULL(D.nro_ctas_amort_pend,0) - IFNULL(C.cuotas_amort_por_vencer,0)) >= {p_cant_min_pag} THEN 1 ELSE 0 END AS flag_resultado_regla
FROM 
    tmp_EXT_tbl_cartdet_crit_ent_ope_prin A 
LEFT JOIN 
    tmp_EXT_tbl_ope_ctas_par  B
ON   
        trim(A.operacion) = trim(B.operacion)
    AND trim(A.sistema) = trim(B.cod_sistema)
LEFT JOIN 
    tmp_EXT_tbl_segmentacion_d00_segmentado C
ON   
       trim(A.operacion) = trim(C.operacion)
    AND trim(A.sistema) = trim(C.sistema)
LEFT JOIN 
    tmp_EXT_tbl_c4_dat_ope_ini_cd D
ON
       trim(A.operacion) = trim(D.num_interno_ident)
    AND trim(A.sistema) = trim(D.sistema)    
...
paso_query70 = f"""DELETE FROM {base_silverX}.tbl_cartdet_crit_sal_ope_campo where fecha_cierre = {FechaX} AND periodo_cierre = {PeriodoX}  AND criterio_salida = {p_GruCritDetSal} AND tipo_proceso = '{tipo_procesoX}' """
...
# MAGIC LEFT JOIN 
# MAGIC     tmp_EXT_tbl_ope_ctas_par  B
# MAGIC ON   
# MAGIC         trim(A.operacion) = trim(B.operacion)
# MAGIC     AND trim(A.sistema) = trim(B.cod_sistema)
# MAGIC LEFT JOIN 
# MAGIC     tmp_EXT_tbl_segmentacion_d00_segmentado C
# MAGIC ON   
# MAGIC        trim(A.operacion) = trim(C.operacion)
# MAGIC     AND trim(A.sistema) = trim(C.sistema)
# MAGIC LEFT JOIN 
# MAGIC     tmp_EXT_tbl_c4_dat_ope_ini_cd D
# MAGIC ON
# MAGIC        trim(A.operacion) = trim(D.num_interno_ident)
# MAGIC     AND trim(A.sistema) = trim(D.sistema)    
# MAGIC WHERE
# MAGIC     trim(B.operacion) IS NOT NULL
# MAGIC     AND trim(B.cod_sistema) IS NOT NULL