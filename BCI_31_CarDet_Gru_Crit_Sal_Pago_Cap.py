...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_CarDet_Gru_Crit_Sal_Pagos_Cap.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 27/09/2022
# MAGIC * Descripcion: Criterio de Salida de Cartera Deteriorada para clientes que tengan amortizacion de capital.
...
#Extrae saldo capital IFRS de la tabla tbl_c4_dat_ope_ini_cd para el ultimo periodo informado
paso_query20 = f"""
...
SELECT
     ...
     A.dv_rut_cliente ,
     A.tipo_operacion,
    'pago_cap' AS nombre_campo,
     IFNULL(B.saldo_total_ifrs,0) AS valor_campo,
     " < "  AS condicion_regla,     
     IFNULL(C.val_skif,0)  AS valor_regla,         
     CASE WHEN IFNULL(B.saldo_total_ifrs,0) < IFNULL(C.val_skif,0) THEN 1 ELSE 0 END AS flag_resultado_regla
FROM 
...
LEFT JOIN 
    tmp_EXT_tbl_c4_dat_ope_ini_cd C
ON
...
# MAGIC %sql
# MAGIC 
# MAGIC --select count(1) from tmp_EXT_tbl_cartdet_crit_ent_ope_prin
# MAGIC 
