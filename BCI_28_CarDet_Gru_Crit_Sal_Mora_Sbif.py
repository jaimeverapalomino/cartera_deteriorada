...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_CarDet_Gru_Crit_Sal_Mora_Sbif.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 21/09/2022
# MAGIC * Descripcion: Criterio de Salida de cartera grupal de clientes sin deuda Sbif periodo actual.
...
# MAGIC %md
# MAGIC ### Clientes con Mora Sbif
# MAGIC --------------------------------------
# MAGIC - Se extrae operaciones para evaluar condiciones de salida

# COMMAND ----------

#Extrae las operaciones de la tabla riesgobdu_golden_db.tbl_segmentacion_cliente_consolidado para el periodo actual
paso_query20 = f"""
...
SELECT 
  ...
  b.dsf_dvcn                        AS dsf_dvcn,
  b.dsf_dvcx                        AS dsf_dvcx,
  b.dsf_cdir                        AS dsf_cdir,
  b.dsf_dmor                        AS dsf_dmor
FROM
  {base_goldX}.tbl_segmentacion_cliente_consolidado b
WHERE 
    fecha_cierre = {FechaX}  
AND periodo_cierre = {PeriodoX}  
AND tipo_proceso = '{tipo_procesoX}'
...
# MAGIC %md
# MAGIC ### Operaciones con Mora Sbif
...
SELECT
     ...
     A.dv_rut_cliente ,
     A.tipo_operacion,
    "dsf_dvcn + dsf_dvcx + dsf_cdir + bdsf_dmor" AS nombre_campo,
    ((B.dsf_dvcn * {p1_VALORDVCN}) + (B.dsf_dvcx * {p1_VALORDVCX}) + (B.dsf_cdir * {p1_VALORCDIR} ) + (B.dsf_dmor * {p1_VALORDMOR} )) AS valor_campo,
    " <= "  AS condicion_regla,
    "{p3_MONTOSBIF}" AS valor_regla,          /* Este valor debe ser parametrico */
    CASE WHEN ((B.dsf_dvcn * {p1_VALORDVCN}) + (B.dsf_dvcx * {p1_VALORDVCX}) + (B.dsf_cdir * {p1_VALORCDIR} ) + (B.dsf_dmor * {p1_VALORDMOR} )) <= {p3_MONTOSBIF} THEN 1 ELSE 0 END AS flag_resultado_regla
FROM 
