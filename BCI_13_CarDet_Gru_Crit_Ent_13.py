...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_13_CarDet_Gru_Crit_Ent_13.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 01/09/2022
# MAGIC * Descripcion: Clientes FAC. Criterio 13.
...
# MAGIC ********************************************************************************************************
# MAGIC ##### Pendientes:
# MAGIC * Se ingesta temporalmente el archivo Factorig
...
paso_query15 = f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_EXT_tbl_cartdet_FAC AS
SELECT
    rut                  AS rut_fac
    ,dv                  AS dv_fac
    ,nope                AS nope_fac
    ,mot                 AS mot_fac
    ,finidet             AS finidet_fac
    ,finimor             AS finimor_fac
    ,dmor                AS dmor_fac
FROM
    tbl_cartdet_FAC
"""
...
paso_query20 =  f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_RES_D00_OPE_FAC_ACT AS 
SELECT  
    ...
    ,IFNULL(A.rut_fac,0)                   AS rut_fac 
	,IFNULL(A.dv_fac,' ')                  AS dv_fac    
	,IFNULL(B.tipo_operacion,' ')          AS tipo_operacion
FROM 
  tmp_EXT_tbl_cartdet_FAC A
LEFT JOIN
  tmp_EXT_tbl_segmentacion_d00_segmentado B
ON
  A.rut_fac = B.rut_cliente
"""
...
SELECT
   ...
   'rut_fac'                         AS nombre_campo,
   cast(A.rut_fac as string)         AS valor_campo,
   " EXIST "                         AS condicion_regla,     
   A.rut_fac                         AS valor_regla,           
   "1"                               AS flag_resultado_regla
FROM 
    tmp_RES_D00_OPE_FAC_ACT A
"""
