...
# MAGIC ### Encabezado
# MAGIC **************************************************************************
# MAGIC * Nombre: BCI_CarDet_Gru_Crit_Sal_Sin_Ref.ipynb
# MAGIC * Ruta: 
# MAGIC * Autor: Gabriel Martinez (SimpleData) - Ing. SW BCI: Jonatan Cancino
# MAGIC * Fecha: 22/09/2022
# MAGIC * Descripcion: Criterio de Salida de Cartera Deteriorada para clientes sin nuevos refinanciamientos.
...
# MAGIC %md
# MAGIC ### Clientes sin refinanciamentos 
# MAGIC --------------------------------------
# MAGIC - Se extrae operaciones para evaluar condiciones de salida

# COMMAND ----------

#Extrae las operaciones de la tabla c4_tmp_gru_ope_cond_ren para el periodo actual con condiciones de renegociado
paso_query20 = f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_EXT_tbl_gru_ope_cond_ren AS
SELECT 
...
#Extrae las operaciones de la tabla c4_tmp_ope_vta_con_crs_baj_mor para el periodo actual con curse bajo mora
paso_query30 = f"""
CREATE OR REPLACE TEMPORARY VIEW tmp_EXT_tbl_ope_vta_con_crs_baj_mor AS
SELECT 
  b.rut_cli                         AS rut_cliente,
  b.num_interno_ident_venta         AS num_interno_ident_venta,
  b.sistema_venta                   AS sistema_venta,  
  b.num_interno_ident_mor           AS num_interno_ident_mor,
  b.sistema_mor                     AS sistema_mor,  
  b.dias_mora                       AS dias_mora
FROM
  tbl_c4_tmp_ope_vta_con_crs_baj_mor b
"""
...
SELECT
    ...
    'operacion - sistema' AS nombre_campo,
     concat(A.operacion,'-',A.sistema,'-',B.operacion,'-',B.cod_sistema,'-',C.num_interno_ident_venta,'-',C.sistema_venta) AS valor_campo,
...
LEFT JOIN 
    tmp_EXT_tbl_gru_ope_cond_ren  B
ON  
    trim(A.rut_cliente) = trim (B.rut_cliente)
    AND trim(A.operacion) = trim(B.operacion)
    AND trim(A.sistema) = trim(B.cod_sistema)
LEFT JOIN 
    tmp_EXT_tbl_ope_vta_con_crs_baj_mor C
ON   
    trim(A.rut_cliente) = trim (C.rut_cliente)
    AND trim(A.operacion) = trim(C.num_interno_ident_venta)
    AND trim(A.sistema) = trim(C.sistema_venta)
WHERE
    trim(B.operacion) IS NULL
    AND trim(C.num_interno_ident_venta) IS NULL
"""
...
# MAGIC      A.sistema,
# MAGIC      A.rut_cliente,
# MAGIC      A.dv_rut_cliente ,
# MAGIC      A.tipo_operacion,
# MAGIC     'operacion - sistema' AS nombre_campo,
# MAGIC      concat(A.operacion,'-',A.sistema) AS valor_campo,
# MAGIC      "IS NULL "  AS condicion_regla,     
# MAGIC      "NULL"  AS valor_regla,           
# MAGIC      CASE WHEN trim (B.operacion) IS NULL THEN 1 ELSE 0 END AS flag_resultado_regla
# MAGIC FROM 
# MAGIC     tmp_EXT_tbl_cartdet_crit_ent_ope_prin A 
# MAGIC JOIN 
# MAGIC     tmp_EXT_tbl_gru_ope_cond_ren  B
# MAGIC ON  
# MAGIC     trim(A.rut_cliente) = trim (B.rut_cliente)
# MAGIC     AND trim(A.operacion) = trim(B.operacion)
# MAGIC     AND trim(A.sistema) = trim(B.cod_sistema)
# MAGIC JOIN 
# MAGIC     tmp_EXT_tbl_ope_vta_con_crs_baj_mor C
# MAGIC ON   
# MAGIC     trim(A.rut_cliente) = trim (C.rut_cliente)
# MAGIC     AND trim(A.operacion) = trim(C.num_interno_ident_venta)
# MAGIC     AND trim(A.sistema) = trim(C.sistema_venta)
...
# MAGIC --8874941
# MAGIC 
# MAGIC --select * from tmp_RES_D00_OPE_CAMPO_EVAL where rut_cliente = 11336144
# MAGIC 
# MAGIC --select * from tmp_EXT_tbl_gru_ope_cond_ren where rut_cliente = 11336144
# MAGIC 
# MAGIC --select * from tmp_EXT_tbl_ope_vta_con_crs_baj_mor where rut_cliente = 11336144
# MAGIC 
